...
function execute_idevice_functions() {
	...
	if ((navigator.userAgent.match(/iPhone/i)) || (navigator.userAgent.match(/iPod/i))) {
		...
		window.addEventListener("load", function() {
			setTimeout(function() {
				window.scrollTo(0, 1);
				// Hide the address bar to increase user experience!
			}, 0);
		});
		/*end hide address*/
...
function setup_all_buttons() {
	...
	$('#theme-switcher ul#theme-links-js li a').bind('click', function(e) {
	...
	$('#theme-switcher ul.mailbox li a').bind('click', function(e) {
		return false;
	});
...
function setup_timepicker() {
	/* default */
	$('.timepicker-input').each(function (index,item){
		$(item).timepicker({
			minuteStep : 1,
			//template : 'modal',
			showSeconds : true,
