...
<div id="header">
    <img onclick="Cookies.remove('qEditMode');window.close();" src="img/cd-icon-close-grey.png" class="headerIconRight iconHover" title="Vider les cookies et fermer">
    <img onclick="reloadImages();" src="img/cd-icon-refresh.png" class="headerIconRight iconHover" title="Recharger les images">
    <img onclick="uploadImg();" src="img/cd-icon-upload-grey.png" class="headerIconCenter iconHover" title="Envoyer une image">
...
<div id="uploadImgDiv" class="lightbox popout">
    ...
    <form action="imgupload.php" method="post" enctype="multipart/form-data" id="uploadImgForm" onsubmit="return checkUpload();">
        ...
        <input type="hidden" name="folder" id="folder" value="<?php echo $useruploadfolder; ?>">
    ...
    <div class="folders" style="text-align: left;padding: 15px;">
        <h3>Choisissez un dossier:</h3>
    <?php
        $dirs= array();
        function buildtree ($dir,&$array){
            $dir = rtrim($dir,'/');
        ...
        buildtree($useruploadpath,$dirs);

        foreach($dirs as $k=>$dir){
            $dir = substr($dir,strpos($dir,$useruploadfolder));
            echo '  <input type="radio" name="folderbase" id="folderbase_'.$k.'" value="'.$dir.'" onclick="chooseFolder(this);" /><label for="folderbase_'.$k.'"> '.$dir.' </label>   <br>';
        }
    ?>
        <form action="createfolder.php" method="post" enctype="multipart/form-data" id="createFolderForm">
            <h3>Créez un dossier:</h3>
            <input type="text" name="foldernew" id="foldernew" value="<?php echo $useruploadfolder; ?>">
            <br>
            <button class="headerBtn greenBtn" name="submit" onclick="$('#createFolderForm').submit();"><img src="img/cd-icon-upload.png" class="headerIcon"> Créer</button>
        </form>
    </div>
</div>
