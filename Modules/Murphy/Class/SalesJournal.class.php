...
class SalesJournal extends FPDF {
	...
	function SalesJournal($begin,$finish,$orientation='P',$unit='mm',$format='A4') {
		parent::__construct($orientation,$unit,$format);
		$this->begin = $begin;
		$this->finish = $finish;
		$this->AcceptPageBreak(true, 12);
		$this->width = array(16,4,15,65,22,11,22,22,8);
		$this->align = array('L','C','C','L','R','R','R','R','L');
	}
	...
	function Header() {
		...
		$tmp .= '                SALES JOURNAL from '.date('d/m/y', $this->begin).' to '.date('d/m/y',$this->finish);
		//$tmp .= '          Currency : '.$this->currency;
		$this->Cell(200, 6, $this->cv($tmp));
	...
	private function colHeader() {
		...
		$header = array('Invoice','T','Date','Client','Tax Excluded','VAT','VAT Amount','Tax Included','Cur');
		$this->SetXY(14, $this->posy);
...
//$GLOBALS["Systeme"]->Log->log("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx:$this->rupAccount:",$inv);
	...
	private function printLine($inv) {
		...
		$this->SetXY(14, $this->posy);
		$this->Cell($this->width[0], 5, $this->cv($inv->Reference), 'L', 0, $this->align[0]);
		$this->Cell($this->width[1], 5, $inv->Type, 'L', 0, $this->align[1]);
		$this->Cell($this->width[2], 5, date('d/m/y',$inv->Date), 'L', 0, $this->align[2]);
		$this->Cell($this->width[3], 5, $this->cv($cli[0]->Company), 'L', 0, $this->align[3]);
		$this->Cell($this->width[4], 5, number_format($inv->TotalTE, 2), 'L', 0, $this->align[4]);
		$this->Cell($this->width[5], 5, number_format($inv->VATRate, 2), 'L', 0, $this->align[5]);
		$this->Cell($this->width[6], 5, number_format($inv->VATAmount, 2), 'L', 0, $this->align[6]);
		$this->Cell($this->width[7], 5, number_format($inv->TotalTI, 2), 'L', 0, $this->align[7]);
		$this->Cell($this->width[8], 5, $this->currency, 'LR', 0, $this->align[8]);
		$this->posy += 5;
		$this->tte += $inv->TotalTE;
		$this->vat += $inv->VATAmount;
	...
	function PrintTotals() {
		...
		$this->Cell($this->width[5], 5, '', 'TBR', 0, $this->align[5]);
		$this->Cell($this->width[6], 5, number_format($this->vat, 2), 'TBR', 0, $this->align[6]);
		$this->Cell($this->width[7], 5, number_format($this->tte, 2), 'TBR', 0, $this->align[7]);
		$this->Cell($this->width[8], 5, $this->currency, 'TBR', 0, $this->align[8]);
		$this->SetFont('Arial','',8);
		$this->total = true;
		$this->tte = 0;
		$this->vat = 0;
