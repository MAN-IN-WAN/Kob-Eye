...
class IncidentClient extends Module {
    ...
    public function Check(){
        ...
        $cliRole = Sys::getOneData('Systeme','Role/Title=[INCIDENT] Client');
        if(!$cliRole){
            $cliRole = genericClass::createInstance('Systeme','Role');
            $cliRole->Title = '[INCIDENT] Client';
            $cliRole->Save();
        }
        $cliGrp->addParent($cliRole);
        $cliGrp->Save();

        $techRole = Sys::getOneData('Systeme','Role/Title=[INCIDENT] Technicien');
        if(!$techRole){
            $techRole = genericClass::createInstance('Systeme','Role');
            $techRole->Title = '[INCIDENT] Technicien';
            $techRole->Save();
        }
        $techGrp->addParent($techRole);
        $techGrp->Save();

        $adminRole = Sys::getOneData('Systeme','Role/Title=[INCIDENT] Administrateur');
        if(!$adminRole){
            $adminRole = genericClass::createInstance('Systeme','Role');
            $adminRole->Title = '[INCIDENT] Administrateur';
            $adminRole->Save();
        }
