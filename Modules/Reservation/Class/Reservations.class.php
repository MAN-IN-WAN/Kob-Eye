...
class Reservations extends genericClass
{
     ...
     */
    static public function getStatsMensuelles($date = 0, $sexe = '', $age = '')
    {
     ...
     * @return  Tableau de données
     */
    static public function getStatsAnnuelles($date = 0, $sexe = '', $age = '')
    {
        $GLOBALS["Systeme"]->connectSQL();
        $data = array();
        if (!$date) $date = time();
     ...
     */
    static private function getBounds($date = 0)
    {
    ...
    public static function getStatsNbSpectacles()
    {
        $GLOBALS["Systeme"]->connectSQL();
        $result = array();
     ...
     */
    public static function getStatsGenre($min, $max, $sexe = '', $age = '')
    {
        $GLOBALS["Systeme"]->connectSQL();
     ...
     * @return  Tableau associatif
     */
    public static function getStatsGenreEvenements($min, $max)
    {
        $GLOBALS["Systeme"]->connectSQL();
     ...
     * @return  Tableau associatif
     */
    public static function getStatsGenreSpectacles($min, $max)
    {
        $GLOBALS["Systeme"]->connectSQL();
     ...
     * @return  void
     */
    public static function getStatsGenrePartProgrammeesReservees($min, $max)
    {

        $GLOBALS["Systeme"]->connectSQL();

        $sql = "SELECT  s.Genre as Nom, SUM(e.NbPlace) as NbProgrammees, SUM(r.NbPlace) as NbReservees
               	FROM `" . MAIN_DB_PREFIX . "Reservation-Spectacle` s
               	INNER JOIN `" . MAIN_DB_PREFIX . "Reservation-Evenement` e ON e.SpectacleId = s.Id
		LEFT JOIN `" . MAIN_DB_PREFIX . "Reservation-ReservationsEvenementId` re ON e.Id = re.EvenementId
		LEFT JOIN `" . MAIN_DB_PREFIX . "Reservation-Reservations` r ON re.Reservations = r.Id
       		WHERE s.Genre != ''
               	AND e.DateDebut >= $min
               	AND e.DateDebut < $max
               	GROUP BY s.Genre
     ...
     * @return  void
     */
    static private function completeGenres($tab)
    {
        if (!is_array(self::$allGenres)) :
            $GLOBALS["Systeme"]->connectSQL();
            $sql = "SELECT DISTINCT ( Genre ) FROM `cult-Reservation-Spectacle` WHERE Genre != '' ORDER BY Genre";
            $req = $GLOBALS["Systeme"]->Db[0]->query($sql);
            self::$allGenres = $req->fetchALL(PDO::FETCH_ASSOC);
        endif;
        foreach (self::$allGenres as $k => $g) :
            $found = false;
            $idx = 0;
            foreach ($tab as $j => $e) :
    ...
    /* Retourne un tableau associatif structure => nombre de résa / nombre de places réservées
      * @return  Tableau associatif
      */
    public static function getTabSpeEveAnneeEncours($date = 0)
    {
        if ($date == 0) {
            $date = time();
            $annee = date('Y', $date);
    ...
    public static function getTabSpeEveAnneePasse()
    {
        $anneePass = date('Y')-1;
        $start = mktime(0, 0, 0, 1, 1, $anneePass);
        $end = mktime(0, 0, 0, 1, 1, $anneePass+1);
     ...
     * @return  Tableau associatif
     */
    public static function getTabVille($min = false, $max = false)
    {
        $GLOBALS["Systeme"]->connectSQL();
    ...
    public static function getStatsPersonneReservantes($min, $max)
    {
...
//echo $sql;
    ...
    // ajout pour futures fonctions stats sur les personnes
    public static function getStatsPersonneReservantesAge($min, $max)
    {
...
//echo $sql;
    ...
    public static function getAllSpeNoEvent()
    {
        $sql = "SELECT i.Id, i.Nom,  i.Debut, i.Fin,  SUM(i.NbEve) as TotEve FROM
			(
				SELECT S.Id ,S.Nom, FROM_UNIXTIME( S.DateDebut ) AS Debut, FROM_UNIXTIME( S.DateFin ) AS Fin, Count( E.Id ) AS NbEve
