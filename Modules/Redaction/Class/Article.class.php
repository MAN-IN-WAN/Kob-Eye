...
class RedactionArticle extends genericClass{
	...
	function genKeyWords() {
		...
		if (is_array($Result))foreach ($Result as $Mc){
			if ($Mc!=""){
				$Tab = Sys::$Modules["Redaction"]->callData("Redaction/BlackList/Titre=".$Mc,"",0,1,"","","COUNT(DISTINCT(m.Id))");
	...
	function Save(){
		...
		$Tab = Sys::$Modules["Redaction"]->callData("Redaction/Article/".$this->Id."/Motclef",0,1000);
