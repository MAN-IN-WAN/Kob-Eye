...
class GDN extends genericClass {
	...
	function GetList($args) {
		...
		$sql = "select distinct $field as word from `##_CEN-GDN` where $field like '$word%' order by word limit 8";
	...
	function GetGDN($args) {
		...
		$sql = "select Id,Paleo,Norma_1,Trad_2,Commentaires,DictionnaireId from `##_CEN-GDN` where $field like '$word%' order by $field,Paleo";
		$sql = str_replace('##_', MAIN_DB_PREFIX, $sql);
		$pdo = $GLOBALS['Systeme']->Db[0]->query($sql);
		$gdn = array();
		$rup = '';
