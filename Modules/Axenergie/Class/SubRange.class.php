...
//		$status = Array(Array("delete",parent::Delete(),$this->Id,$this->Module,$this->ObjectType));
	...
	static function search($db,$target){
		$c = Sys::$Modules["Axenergie"]->callData("Database/".$db->Id."/SubRange/*/Categorie.CategorieId(".$target->Id.")");
		if (sizeof($c)){
	...
	static function add($db,$target,$args,$recurs=true,$auto = false){
		...
		if (is_object($c2)){
			...
			$sr = genericClass::createInstance("Axenergie","SubRange");
			...
			if (isset($p[0])&&is_object($p[0])){
				...
				//Categorie racine
				$c = genericClass::createInstance('Axenergie','SubRange');
				$c->Id = 0;
				$sr->AddParent($db);
			}
		...
		if ($recurs){
			...
			$m = Sys::$Modules["Axenergie"]->callData("Categorie/".$target->Id."/Categorie/Affiche=0".(($auto)?"&Prive=1":""));
			if (is_array($m))foreach ($m as $mo){
				$mo = genericClass::createInstance("Axenergie",$mo);
	...
	static function remove($db,$target){
		...
		$O = Sys::$Modules["Axenergie"]->callData("Database/".$db->Id."/SubRange/*/Categorie.CategorieId(".$target->Id.")");
