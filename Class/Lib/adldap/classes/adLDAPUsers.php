...
class adLDAPUsers {
    ...
    */
    public function info($username, $fields = NULL, $isGUID = false)
    {
        ...
        if ($fields === NULL) { 
            $fields = array("samaccountname","mail","memberof","department","displayname","telephonenumber","primarygroupid","objectsid","physicaldeliveryofficename","firstname","surname"); 
        }
    ...
    * @return array
    */
    public function find($includeDescription = false, $searchField = false, $searchFilter = false, $sorted = true,$selectFields = Array()){
        if (!$this->adldap->getLdapBind()){ return false; }
        ...
        $filter = "(&(objectClass=user)(samaccounttype=" . adLDAP::ADLDAP_NORMAL_ACCOUNT .")(objectCategory=person)(physicaldeliveryofficename=*)" . $searchParams . ")";
        $fields=array("samaccountname","displayname","physicaldeliveryofficename");
		//if (sizeof($selectFields))
		 //	$fields = $selectFields;
        $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter, $fields);
        $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);
return $entries;
