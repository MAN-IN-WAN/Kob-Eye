...
class ComptageRequest implements ArrayAccess
{
    ...
    public function listInvalidProperties()
    {
        ...
        $allowed_values = $this->getComptageAllowableValues();
        if (!in_array($this->container['comptage'], $allowed_values)) {
            $invalid_properties[] = sprintf(
                "invalid value for 'comptage', must be one of '%s'",
        ...
        $allowed_values = $this->getSmslongAllowableValues();
        if (!in_array($this->container['smslong'], $allowed_values)) {
            $invalid_properties[] = sprintf(
                "invalid value for 'smslong', must be one of '%s'",
                implode("', '", $allowed_values)
            );
        }
    ...
    {
        $allowed_values = $this->getSmslongAllowableValues();
        if (!is_null($smslong) && !in_array($smslong, $allowed_values)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'smslong', must be one of '%s'",
                    implode("', '", $allowed_values)
