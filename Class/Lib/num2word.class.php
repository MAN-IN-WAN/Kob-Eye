...
class num2word {
	...
	private $units =
			array(
				...
				"USD" => array(
					100,
					"fr-FR" => array('dollar', 'dollars', 'cent', 'cents', '', ''),
					"pt-PT" => array('dollar', 'dollars', 'cêntimo', 'cêntimos', 'm', 'm'),
					"en-EN" => array('dollar', 'dollars', 'cent', 'cents', '')
				),
				"GBP" => array(
					100,
					"fr-FR" => array('livre streling', 'livres sterling', 'penny', 'pence', '', ''),
					"pt-PT" => array('dollar', 'dollars', 'cêntimo', 'cêntimos', 'm', 'm'),
					"en-EN" => array('sterling pound', 'sterling pounds', 'penny', 'pence', '')
				),
				"AUD" => array(
					100,
					"fr-FR" => array('dollar autralien', 'dollars australiens', 'cent', 'cents', '', ''),
					"pt-PT" => array('dollar', 'dollars', 'cêntimo', 'cêntimos', 'm', 'm'),
					"en-EN" => array('australian dollar', 'australian dollars', 'cent', 'cents', '')
				),
				"NZD" => array(
					100,
					"fr-FR" => array('dollar néo-zélandais', 'dollars néo-zélandais', 'cent', 'cents', '', ''),
					"pt-PT" => array('dollar', 'dollars', 'cêntimo', 'cêntimos', 'm', 'm'),
					"en-EN" => array('New Zeland dollar', 'New Zeland dollars', 'cent', 'cents', '')
				),
				"ZAR" => array(
					100,
					"fr-FR" => array('rand', 'rands', 'cent', 'cents', '', ''),
					"pt-PT" => array('dollar', 'dollars', 'cêntimo', 'cêntimos', 'm', 'm'),
					"en-EN" => array('rand', 'rand', 'cent', 'cents', '')
				),
				"EUR" => array(
					100,
					"fr-FR" => array('euro', 'euro', 'cent', 'cents', '', ''),
					"pt-PT" => array('euro', 'euro', 'cêntimo', 'cêntimos', 'm', 'm'),
					"en-EN" => array('euro', 'euro', 'cent', 'cents', '', '')
				),
				"t"  => array(
					1000,
	...
	function __construct($nb, $unit){
		...
		if (num2word::DEBUG) echo "construct : [" . $this->nb . "]";
	}
	...
	private function getThree($group, $unit, $language, $gender){
		...
		if (num2word::DEBUG) echo "<br>local a [" . $return . "] ";
		// Exceptions.
		...
		if (num2word::DEBUG) echo "<br>local b $unit [" . $return . "] ";
		// Exceptions de genre.
		...
		if (num2word::DEBUG) echo "<br>local c [" . $return . "] ";
		return $return;
	}
	...
	private function part($nb, $language, $gender){
		...
		if (num2word::DEBUG) echo "<br>part a : [" . $return . "]";
		// Exceptions.
		...
		if (num2word::DEBUG) echo "<br>part b : [" . $return . "]";
		return $return;
	}
	 ...
	 * @param string $language Langue à utiliser pour la sortie.
	 */
	function convert($language, $sep=''){
		// Partie entière.
		$return = $this->part($this->parts[0], $language, 4);
		$unit = ($this->parts[0] > 1) ? $this->units[$this->unit][$language][1] : $this->units[$this->unit][$language][0];
		if($unit) $return .= ' '.$unit;
		...
		if (count($this->parts) == 2){
			$return .= ($sep ? $sep : $this->separators[$language][0]) . $this->part($this->parts[1], $language, 5);
			$unit = ($this->parts[1] > 1) ? $this->units[$this->unit][$language][3] : $this->units[$this->unit][$language][2];
			if($unit) $return .= ' '.$unit;
		}
		
		if (num2word::DEBUG) echo "<br>";
		return $return;
	}
}